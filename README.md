...
3. 项目的 Project URL 与 publishable key 已预置在看板中，无需在每台设备上重复填写。
4. 在电脑和手机使用同一个邮箱账户登录。应用会把每个账户的数据保存为一行，并在联网时自动同步；离线修改会先保存在本机。首次发现两台设备都有未同步修改时，会要求选择云端版本或本机版本，避免静默覆盖。
5. 注册确认与密码重置邮件会固定回到线上看板；忘记看板密码时，可在“手机与离线”区域发送重置邮件。
...
Publishable/anon 公钥可以出现在浏览器中，真正的访问边界是 `study_states` 表的 RLS 策略。不要把 secret/service-role key、数据库密码或任何私密密钥填入网页或提交到仓库。
